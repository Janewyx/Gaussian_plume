...
## computing Gaussian concentration for each class
gau_df <- gau_df %>% 
  group_by(class) %>% 
  mutate(conc = 1000000*5/(2*pi*1*sigmay*sigmaz)*(exp((-(z-Effective.Stack.Height)^2)/(2*sigmaz^2)) + exp(-(z+Effective.Stack.Height)^2/(2*sigmaz^2)))*exp((0)))
